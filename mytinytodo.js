...
(function () {
    ...
    var tabLists = {
        ...
        exists: function (id) {
            if (this._lists[id] || id == -1) return true; else return false;
        },
        add: function (list) {
        ...
        get: function (id) {
            if (id == -1) return this._alltasks; else return this._lists[id];
        },
        getAll: function () {
            var r = [];
            for (var i in this._order) {
                r.push(this._lists[this._order[i]]);
            }
            return r;
        },
    ...
    var mytinytodo = window.mytinytodo = _mtt = {
        ...
        init: function (options) {
            ...
            $("#duedate").datepicker({
                ...
                showOn: 'button',
                buttonImage: _mtt.templateUrl + 'images/calendar.png',
    ...
    function publishCurList() {
        if (!curList) return false;
        _mtt.db.request('publishList', {
            list: curList.id,
            publish: curList.published ? 0 : 1
        }, function (json) {
            if (!parseInt(json.total)) return;
    ...
    function toggleAllNotes(show) {
        ...
        if (_mtt.options.saveShowNotes) _mtt.db.request('setShowNotesInList', {
            list: curList.id,
            shownotes: show
        }, function (json) {
        });
        ...
        _mtt.db.request('completeTask', {id: id, compl: compl, list: curList.id}, function (json) {
            ...
            if (item.compl && !curList.showCompl) {
                ...
                $('#taskrow_' + id).fadeOut('fast', function () {
                    changeTaskOrder(id);
                    $(this).effect("highlight", {color: _mtt.theme.editTaskFlashColor}, 'normal', function () {
                        $(this).css('display', '')
                    });
                });
            }
    ...
    function mttMenu(container, options) {
        ...
        this.show = function (caller) {
            ...
            this.$container.css({
                ...
                top: y,
                left: x,
                width: this.$container.width() /*, 'min-width': $caller.width()*/
            }).show();
            var menu = this;
        ...
        this.showSub = function () {
            ...
            this.$container.css({
                ...
                top: y,
                left: x,
                width: this.$container.width() /*, 'min-width': this.$caller.outerWidth()*/
            }).show();
            this.menuOpen = true;
        };
    ...
    */
    ...
    function doAuth(form) {
        $.post(mytinytodo.mttUrl + 'ajax.php?login', {
            login: 1,
